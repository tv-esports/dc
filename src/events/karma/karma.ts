...
// export default class MessageCreateKarmaEvent extends BaseEvent {
//     constructor() {
//         super("messageCreate");
//     }
...
//         const owners = ["578678204890349594", "330101106678169610"]
//         // check if the user is a developer
//         const owner = owners.includes(message.author.id);
//         if (owner) return;

//         const threeDaysAgo = new Date(Date.now() - 3 * 24 * 60 * 60 * 1000);

//         const recentKarmaCase = await KarmaModel.findOne({
//             karmaUserID: message.author.id,
//         }).sort({ inserted_at: -1 });

//         const karmaChannel = message.guild.channels.cache.get(process.env.LOTTERY_CHAT) as TextChannel;
...
//         const containsKarmaWord = karmaWords.some((word) => message.content.toLowerCase().includes(word));

//         if (containsKarmaWord && recentKarmaCase && recentKarmaCase.inserted_at > threeDaysAgo) return message.delete();

//         const predefinedPenalties: { [key: string]: string[] } = {
//             "Timeout": [],
//             "Nickname": [],
//             "XP": []
//         };

//         if (containsKarmaWord && (!recentKarmaCase || recentKarmaCase.inserted_at < threeDaysAgo)) {
//             const karmaEmbed = new EmbedBuilder()
//                 .setTitle("Karma Crime Case")
//                 .setDescription(`A new karma crime case has been opened. You got 10 minutes to vote for a penalty that <@${message.author.id}> should receive for saying a bad word.`)
//                 .addFields({
//                     name: "**Timeout**",
//                     value: "The user receives a 10min Timeout (unable to do anything on the server)"
//                 }, {
//                     name: "**XP**",
//                     value: "The user loses min. 1, max. 3 levels (depending on his level)"
//                 }, {
//                     name: "**Nickname**",
//                     value: "The user receives a weird nickname (randomly generated)"
//                 })
//                 .setColor("Red")
//                 .setTimestamp()
...
//                 .setCustomId("karma-timeout")
//                 .setLabel("Timeout")
//                 .setStyle(ButtonStyle.Secondary);

//             const previousXPLevelButton = new ButtonBuilder()
//                 .setCustomId("karma-previous-xp")
//                 .setLabel("XP")
//                 .setStyle(ButtonStyle.Secondary);

//             const nicknameButton = new ButtonBuilder()
//                 .setCustomId("karma-nickname")
//                 .setLabel("Nickname")
//                 .setStyle(ButtonStyle.Secondary);

//             const karmaRow = new ActionRowBuilder<ButtonBuilder>()
//                 .addComponents(timeoutButton, previousXPLevelButton, nicknameButton);

//             await KarmaModel.create({
//                 karmaUserID: message.author.id,
//                 karmaGuildID: message.guild.id,
//                 karmaMessageID: message.id,
//                 karmaStatus: "open",
//                 karmaPunishments: predefinedPenalties,
//                 karmaDecision: "",
//                 inserted_at: new Date(),
//                 updated_at: new Date()
//             });

//             const karmaReportMessage = await karmaChannel.send({ embeds: [karmaEmbed], components: [karmaRow] });

//             // setTimeout(async () => {
//             //     const recentCase = await KarmaModel.findOne({
//             //         karmaUserID: message.author.id,
//             //     }).sort({ inserted_at: -1 });

//             //     if (recentCase && Object.values(recentCase.karmaPunishments).every(penalty => penalty.length === 0)) {
//             //         const resultEmbed = new EmbedBuilder()
//             //             .setTitle("Karma Crime Case")
//             //             .setDescription(`The karma crime case has been closed!\n\nNo one voted, so the case was closed without a decision.`)
//             //             .setColor("Red")
//             //             .setFooter({ text: `${message.author.id}` });
...
//             //         await karmaReportMessage.edit({ embeds: [resultEmbed], components: [] });
//             //     }
//             // }, 1000 * 60 * 10);
//         }
//     }
